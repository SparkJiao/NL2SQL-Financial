...
logger = get_child_logger("sqliteStructureTrans")
...
def gen_sqlite_struct_file_from_list(predictions):
    """生成sqlite文件"""
    sqlite_struct_list = []
    for j in range(len(predictions)):
        q_idj = int(predictions[j]["q_id"])
        sql_queryj = predictions[j]["sql_query"]
        questionj = predictions[j]["question"]
        db_namej = predictions[j]["db_name"]
        try:
            res_dict = sqlite_2_struct(questionj, q_idj, sql_queryj, db_namej)
        except Exception as e:
            logger.info("q_id: {}, error: {}".format(q_idj, e))
            # print("q_id: {}, error: {}".format(q_idj, e))
            res_dict = {
                "q_id": q_idj,
                "question": questionj,
                "db_name": db_namej,
...
if __name__ == '__main__':
    ...
    # predictions = [{
    #     "q_id": 1,
    #     "question": "统计出最高学历为博士或博士后的基金经理总共有多少",
    #     "db_name": "",
    #     "sql_query": "select count(a.name) from mf_fundmanagernew as a join mf_personalinfo as b on a.personalcode = b.personalcode where b.education = '博士' or a.education = '博士' or a.education = '博士'",
    # }]
    predictions = [{
        "q_id": 116,
        "question": "显示指标周期为\"一个月\"的基金收益为正的基金数目，按基金运作形式分组展示",
        "db_name": "ccks_fund",
        "sql_query": "select b.type, count(*) from mf_fundreturnrank as a join mf_fundarchives as b on a.innercode = b.innercode where a.indexcycle = '一个月' and a.fundreturn > 0 group by b.type",
    }]
    print(gen_sqlite_struct_file_from_list(predictions))
